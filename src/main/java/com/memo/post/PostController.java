...
public class PostController {
	...
	public String postListView(
		...
		int prevId = 0;
		int nextId = 0;
		if (postList.isEmpty() == false) {  // 글목록이 비어있지 않을때 페이징 정보 세팅
			prevId = postList.get(0).getId(); // 첫번째칸 id
			nextId = postList.get(postList.size() - 1).getId(); // 마지막칸 id
			
			// 이전 방향의 끝인가? 그러면 0으로 세팅
			// 지금 들어있는 prevId가 테이블의 제일 큰 숫자와 같으면 끝 페이지라는 뜻(더이상 '이전'을 눌러 갈 곳이 없다)
			if (postBO.isPrevLastPageByUserId(userId, prevId)) {
				prevId = 0;
			}
			
			// 다음 방향의 끝인가? 그러면 0으로 세팅
			// 지금 셋팅해놓은 nextId와 테이블의 제일 작은 숫자와 같으면 끝 페이지라는 뜻(더이상 '다음'을 눌러 갈 곳이 없다)
